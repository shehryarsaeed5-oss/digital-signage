...
const SLOT_CONFIG = [
  { id: 'now-slot-1', type: 'nowShowing' },
  { id: 'now-slot-2', type: 'nowShowing' },
  { id: 'coming-slot-1', type: 'comingSoon' },
  { id: 'now-slot-3', type: 'nowShowing' },
  { id: 'now-slot-4', type: 'nowShowing' },
  { id: 'coming-slot-2', type: 'comingSoon' },
  { id: 'now-slot-5', type: 'nowShowing' },
  { id: 'now-slot-6', type: 'nowShowing' },
  { id: 'coming-slot-3', type: 'comingSoon' },
];

const NOW_SHOWING_PAGE_SIZE = 6;
const COMING_SOON_PAGE_SIZE = 3;

const playerNode = document.getElementById('cinema-player');
const statusNode = document.getElementById('cinema-status');
const clockNode = document.getElementById('cinema-clock');
const nowShowingSlotNodes = SLOT_CONFIG
  .filter((slot) => slot.type === 'nowShowing')
  .map((slot) => document.getElementById(slot.id));
const comingSoonSlotNodes = SLOT_CONFIG
  .filter((slot) => slot.type === 'comingSoon')
  .map((slot) => document.getElementById(slot.id));
...
const PLAYER_CACHE_KEY = 'signage_cache_cinema';
...
function startPlaybackWatchdog() {
  ...
  state.playbackWatchdogTimer = window.setInterval(() => {
    ...
    console.warn('Playback watchdog triggered; reloading cinema player.');
    location.reload();
  }, 15000);
}
...
function renderComingSoonCard(movie) {
  ...
  card.innerHTML = `
    ...
    <div class="cinema-card-info">
      ...
      <div class="cinema-card-bottom">
        <div class="cinema-release-info">
          <span class="cinema-release-label">Releasing</span>
          <div class="cinema-release-date">${escapeHtml(movie.releaseDate || 'To Be Announced')}</div>
        </div>
      </div>
    </div>
  `;
...
function applyPlayerSettings(settings) {
  ...
  const appearanceVariables = [
    ['cinema_wall_gap_px', '--cinema-wall-gap', 'px', 30],
    ['cinema_wall_padding_px', '--cinema-wall-padding', 'px', 30],
    ['cinema_wall_title_font_rem', '--cinema-wall-title-size', 'rem', 2.2],
  ...
  appearanceVariables.forEach(([settingKey, cssVar, unit, fallback]) => {
    const numericValue = Number(settings?.[settingKey]);
...
function isAdsEnabledForScreen(settings) {
  return settings?.ads_enabled !== false
    ...
    && settings?.cinema_ads_enabled !== false;
}
...
function isAdTargetedToScreen(ad, screen) {
  ...
  if (targets.length === 0) return true; // empty targets => all screens
  return targets.includes(normalizedScreen);
}

function normalizeMoviePlaylistItems(nowShowingMovies, comingSoonMovies) {
  return [
    ...(Array.isArray(nowShowingMovies) ? nowShowingMovies : []),
    ...(Array.isArray(comingSoonMovies) ? comingSoonMovies : []),
  ].map((movie) => ({
    ...movie,
    playlistType: 'movie',
  }));
...
function createGlobalEmptyState() {
  ...
  emptyState.innerHTML = `
    <div class="cinema-wall-empty-copy">
      <div class="cinema-wall-empty-kicker">Cinema Wall</div>
      <h1 class="cinema-wall-empty-title">Schedule Unavailable</h1>
      <p class="cinema-wall-empty-text">Movie data is not available right now. Please check the cinema schedule feed and try again.</p>
    </div>
  `;
  return emptyState;
}
...
function buildCinemaWallAssignments() {
  ...
  const comingPool = Array.isArray(state.comingSoonMovies) ? state.comingSoonMovies : [];
  const effectiveNowPool = nowPool.length > 0 ? nowPool : comingPool;
  const effectiveComingPool = comingPool.length > 0 ? comingPool : nowPool;

  return {
    effectiveNowPool,
    effectiveComingPool,
    nowShowingTiles: getPagedTileAssignments(effectiveNowPool, NOW_SHOWING_PAGE_SIZE, state.nowShowingPage),
    comingSoonTiles: getPagedTileAssignments(effectiveComingPool, COMING_SOON_PAGE_SIZE, state.comingSoonPage),
  };
}

function renderCinemaWall() {
  const hasNowShowing = state.nowShowingMovies.length > 0;
  const hasComingSoon = state.comingSoonMovies.length > 0;

  if (!hasNowShowing && !hasComingSoon) {
    clearWallTileSet(nowShowingSlotNodes);
    clearWallTileSet(comingSoonSlotNodes);
  ...
  const { nowShowingTiles, comingSoonTiles } = buildCinemaWallAssignments();
  ...
  comingSoonSlotNodes.forEach((container, index) => renderSingleCell(container, comingSoonTiles[index], renderComingSoonCard));
}

function scheduleSectionRotation(sectionKey) {
  const { effectiveNowPool, effectiveComingPool } = buildCinemaWallAssignments();

  if (sectionKey === 'nowShowing') {
    ...
    state.nowShowingTimer = window.setTimeout(() => {
      ...
      renderCinemaWall();
      scheduleSectionRotation('nowShowing');
    }, Math.max(1, state.playerSettings.now_showing_duration_seconds) * 1000);
    return;
  }

  clearRotationTimer('comingSoonTimer');

  if (effectiveComingPool.length <= COMING_SOON_PAGE_SIZE) {
    return;
  }

  state.comingSoonTimer = window.setTimeout(() => {
    state.comingSoonPage += 1;
    renderCinemaWall();
    scheduleSectionRotation('comingSoon');
  }, Math.max(1, state.playerSettings.coming_soon_duration_seconds) * 1000);
}

function applyCinemaBoards(nowShowingMovies, comingSoonMovies) {
  state.nowShowingMovies = Array.isArray(nowShowingMovies) ? nowShowingMovies : [];
  state.comingSoonMovies = Array.isArray(comingSoonMovies) ? comingSoonMovies : [];

  const effectiveNowPool = state.nowShowingMovies.length > 0 ? state.nowShowingMovies : state.comingSoonMovies;
  const effectiveComingPool = state.comingSoonMovies.length > 0 ? state.comingSoonMovies : state.nowShowingMovies;

  const nowShowingPageCount = Math.max(1, Math.ceil(effectiveNowPool.length / NOW_SHOWING_PAGE_SIZE));
  const comingSoonPageCount = Math.max(1, Math.ceil(effectiveComingPool.length / COMING_SOON_PAGE_SIZE));

  state.nowShowingPage = state.nowShowingPage % nowShowingPageCount;
  state.comingSoonPage = state.comingSoonPage % comingSoonPageCount;

  console.info('[Cinema Wall] 6+3 mapping', {
    nowShowingPool: state.nowShowingMovies.length,
    comingSoonPool: state.comingSoonMovies.length,
  });
...
async function fetchCinemaMovies() {
  try {
    ...
    console.log('fresh data loaded', { player: 'cinema' });
    ...
    console.error('Failed to load cinema movies', error);
    const cachedPayload = readPlayerCache();
    if (cachedPayload && cachedPayload.data !== undefined) {
      console.log('cached data used', { player: 'cinema' });
    ...
    console.log('no cache available', { player: 'cinema' });
